...
import { getSchoolFeeByID } from './schoolfeeActions';
import { getSchoolByID } from "../school/schoolActions";
import { getList as getListTypes } from './../admin/feetype/feetypeActions';

class SchoolFeeForm extends Component {
	constructor() {
	 super();
		this.state = {
			school: {}
	...
	componentWillMount() {
		...
		if(id){
		const schoolContent = this.props.getSchoolFeeByID(id)
		    .then((res) =>{
			...
			<form role='form' onSubmit={handleSubmit}>
				<div className='box-body'>
					<Row>
						...
						<Field name='type_id' pupolate={selectTypes} component={LabelAndSelect} label='SchoolFeesTypes' cols='12 3' readOnly={readOnly} option='name'/>
...
SchoolFeeForm = reduxForm({form: 'schoolFeeForm', destroyOnUnmount: false})(SchoolFeeForm);
const mapStateToProps = state => ({schoolfee: state.schoolfee.listFilter, listTypes: state.feetype.list});
const mapDispatchToProps = dispatch => bindActionCreators({getSchoolByID, getSchoolFeeByID,  getListTypes}, dispatch);
export default connect(mapStateToProps, mapDispatchToProps)(SchoolFeeForm);
