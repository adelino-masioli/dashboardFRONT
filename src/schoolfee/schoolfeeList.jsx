...
class SchoolFeeList extends Component {
		...
		if (list.status == false){
			...
			return list.map(res => (
				<tr key={res.id}>
					...
					<td>{res.type.name ? res.type.name : '-'}</td>
					<td>{res.name}</td>
					...
					<td className='text-center'>
						<div className='btn-group'>
							<Link to={`/school-fees/${id}/${res.id}/update`} className='btn btn-xs btn-flat btn-warning' onClick={() => this.props.showUpdate(res)}>
								<i className='fa fa-pencil'></i>
							</Link>
							<Link to={`/school-fees/${id}/${res.id}/delete`} className='btn btn-xs btn-flat btn-danger' onClick={() => this.props.showDelete(res)}>
	...
	renderHeader(){
		const headerTitle = ['ID', 'TYPE', 'NAME',  'DESCRIPTION', 'PRICE', 'NOTES', 'STATUS', 'ACTIONS'];
	...
	render() {
		...
		return (
			...
			<Row>
			  <Grid cols='12 12'>
			  	<Box>
					<Link to={`/school-fees/${id}/create`} className='btn btn-sm btn-flat btn-primary pull-right' onClick={() => this.props.showCreate()}>
...
const mapStateToProps = state => ({list: state.schoolfee.list});
const mapDispatchToProps = dispatch => bindActionCreators({getList, showCreate, showUpdate, showDelete}, dispatch);
export default connect(mapStateToProps, mapDispatchToProps)(SchoolFeeList);
