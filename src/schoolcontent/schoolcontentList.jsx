...
class SchoolContentList extends Component {
		...
		if (list.status == false){
			...
			return list.map(res => (
				<tr key={res.id}>
					...
					<td>{res.language.name}</td>
					<td>{res.description}</td>
					<td>{res.notes}</td>
					<td>{res.status == 1 ? 'Active' : 'No active'}</td>
					<td className='text-center'>
						<div className='btn-group'>
							<Link to={`/school-contents/${id}/${res.id}/update`} className='btn btn-xs btn-flat btn-warning' onClick={() => this.props.showUpdate(res)}>
								<i className='fa fa-pencil'></i>
							</Link>
							<Link to={`/school-contents/${id}/${res.id}/delete`} className='btn btn-xs btn-flat btn-danger' onClick={() => this.props.showDelete(res)}>
	...
	renderHeader(){
		const headerTitle = ['ID', 'LANGUAGE', 'DESCRIPTION', 'NOTES', 'STATUS', 'ACTIONS'];
	...
	render() {
		...
		return (
			...
			<Row>
			  <Grid cols='12 12'>
			  	<Box>
					<Link to={`/school-contents/${id}/create`} className='btn btn-sm btn-flat btn-primary pull-right' onClick={() => this.props.showCreate()}>
...
const mapStateToProps = state => ({list: state.schoolcontent.list});
