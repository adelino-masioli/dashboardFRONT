...
export function getList(){
	const request = axios.get(`${BASE_URL}/companies`, {"headers": {"authorization": "Bearer "+token()}});
	return {
		type: 'LIST_COMPANY_FETCHED',
		payload: request
	};
}
//create new register
...
function submit(values, method){
	return dispatch => {
		...
		const url = method === 'post' ? `${BASE_URL}/companies` : `${BASE_URL}/companies/${id}`; 
