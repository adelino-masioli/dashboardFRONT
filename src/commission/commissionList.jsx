...
class CommissionList extends Component {
				...
				<tr key={res.id}>
					...
					<td className='text-center'>
						<div className='btn-group'>
							<Link to={`/commissions/${id}/${res.id}/update`} className='btn btn-xs btn-flat btn-warning' onClick={() => this.props.showUpdate(res)}>
								<i className='fa fa-pencil'></i>
							</Link>
							<Link to={`/commissions/${id}/${res.id}/delete`} className='btn btn-xs btn-flat btn-danger' onClick={() => this.props.showDelete(res)}>
	...
	renderHeader(){
		const headerTitle = ['ID', 'VALUE', 'COMPANIES',  'ITEM TYPE', 'STATUS', 'ACTIONS'];
	...
	render() {
		...
		return (
			...
			<Row>
			  <Grid cols='12 12'>
			  	<Box>
					<Link to={`/commissions/${id}/create`} className='btn btn-sm btn-flat btn-primary pull-right' onClick={() => this.props.showCreate()}>
...
const mapStateToProps = state => ({list: state.commission.list});
