...
class SchoolContactList extends Component {
				...
				<tr key={res.id}>
					...
					<td className='text-center'>
						<div className='btn-group'>
							<Link to={`/school-contacts/${id}/${res.id}/update`} className='btn btn-xs btn-flat btn-warning' onClick={() => this.props.showUpdate(res)}>
								<i className='fa fa-pencil'></i>
							</Link>
							<Link to={`/school-contacts/${id}/${res.id}/delete`} className='btn btn-xs btn-flat btn-danger' onClick={() => this.props.showDelete(res)}>
	...
	renderHeader(){
		const headerTitle = ['ID', 'NAME', 'OCCUPATION', 'PHONE NUMBER', 'SKYPE', 'NOTES', 'STATUS', 'ACTIONS'];
	...
	render() {
		...
		return (
			...
			<Row>
			  <Grid cols='12 12'>
			  	<Box>
					<Link to={`/school-contacts/${id}/create`} className='btn btn-sm btn-flat btn-primary pull-right' onClick={() => this.props.showCreate()}>
...
const mapStateToProps = state => ({list: state.schoolcontact.list});
const mapDispatchToProps = dispatch => bindActionCreators({getList, showCreate, showUpdate, showDelete}, dispatch);
export default connect(mapStateToProps, mapDispatchToProps)(SchoolContactList);
